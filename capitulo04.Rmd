...
2. Como las modificaciones a las cifras presupuestarias continúan, incluso en los años siguientes al cierre del ejercicio, se prefiere utilizar las exposiciones de motivos que contengan el año de interés como el primero de la serie de cinco años. Al operar de esta forma se cuenta con cifras más estables, de modo que si se quiere presentar, por ejemplo los datos del año 1998, se opta por elegir la exposición de motivos del año 2003, porque contiene la información de los presupuestos de los cinco años anteriores, de 1998 a 2002. Es más probable que para la elaboración del proyecto de ley de presupuesto del año 2003, los datos de 1998 permanezcan sin modificaciones. Este criterio se aplica para los años de 1998 hasta 2011, luego simplemente se utiliza la última exposición de motivos publicada, es decir la del año 2016. 
...
library(knitr)
library(kableExtra)
Prop_Ppto<-read.csv("PptoEdoMun.csv",sep = ";",header=TRUE,skip=1,encoding="UTF-8")
kable(Prop_Ppto,format="latex",booktabs =TRUE, caption='Ingresos propios vs totales de estados y municipios, 2004-2015',align="r")%>%kable_styling(latex_options=c("scale_down","HOLD_position"))%>%add_header_above(c(" "=1,"Estados"=3,"Municipios"=3))
```
...
library(ggfittext)
library(ggrepel)
Ppto2011v2<-read.csv("Ppto2011v2.csv",header = TRUE,sep = ";",encoding="UTF-8")
par(mfrow=c(2,1))
ggplot(as.data.frame(Ppto2011v2),out.width = 0.5,
  ...
  geom_fit_text(stat = "stratum", width = 1/4, min.size = 0.1,infer.label = TRUE) +
...
library(png)
include_graphics("figurasR/Aluvial2-1",auto_pdf = TRUE )
```
...
library(ggplot2)
library(dplyr)
Sec_Nac <- read.csv("Sec_Nac.csv", sep=";",dec = ",",encoding="UTF-8")
...
\begin{equation}
Valor\; real= \frac{Valor\; nominal\times \;\acute{\imath}ndice\; de\; inflaci\acute{o}n\; actual}{\acute{\imath}ndice\; de\; inflaci\acute{o}n\; del\; a\tilde{n}o\; previo\; o\; a\tilde{n}o\; base}
\end{equation}
...
```{r GCorriente, echo=FALSE, caption=TRUE}
Gastos_C<-read.csv("GC_2008-2016.csv",header = TRUE,sep = ";",row.names = 1,encoding="UTF-8")
...
\begin{equation}
Valor\; real\;mes_i= \frac{Valor\; nominal\;mes_i\times \;\acute{\imath}ndice\; de\; inflaci\acute{o}n\; del\;mes_i}{\acute{\imath}ndice\; de\; inflaci\acute{o}n\; del\; mes\, base}
\end{equation}
...
con 
\begin{equation}
f_i=\frac{\acute{\imath}ndice\; de\; inflaci\acute{o}n\; del\;mes_i}{\acute{\imath}ndice\; de\; inflaci\acute{o}n\; del\; mes\, base}
\end{equation}
...
```{r ReexpresionMes, echo=FALSE, out.width = "50%", out.height="30%",fig.align='center',fig.cap ="Esquema de reexpresión mensual del presupuesto."}
knitr::include_graphics("figurasR/ReexpresionMes",auto_pdf = TRUE)
```
...
\begin{equation}
Presupuesto\;real\;del\;a\tilde{n}o_j=\sum_{i=1}^{12} \frac{Presupuesto\; nominal\;del\;a\tilde{n}o_j}{12}\times \;f_i \quad con\quad j: a\tilde{n}os
\end{equation}
...
```{r nomcolumna, echo=FALSE}
Cultura_estados00<-read.csv("Cultura Estados.csv",header = TRUE,sep = ";",row.names = 1,encoding="UTF-8")
...
```{r BPnominal, echo=FALSE, fig.cap="Presupuesto Cultural Estadal en términos nominales, período 2008-2018", fig.pos="H"}
...
```{r BPescalado, fig.cap="Presupuesto Cultural Estadal 2008-2018 escalado", fig.pos="H"}
...
```{r SinAtipicos, fig.cap="Presupuesto cultural de los estados para años sin valores atípicos", fig.pos="H"}
par(mfrow=c(2,2))
...
```{r MasAtipicos, fig.cap="Presupuesto cultural de los estados para años con los mayores valores atípicos", fig.pos="H"}
par(mfrow=c(2,2))
...
```{r BPreexpresado, fig.cap="Presupuesto Cultural Estadal 2008-2018 a valores reales", fig.pos="H"}
...
```{r BPmedian, fig.cap="Presupuesto Cultural Estadal 2008-2018 en base a mediana de 2008", fig.pos="H"}
reex2<-100*reex/m
...
Se observa una caída sostenida, y más abrupta a partir de 2013, de una base de 100 a solo 0,5 entre 2008 y 2018 en el aporte estadal a la cultura. Una reducción de 200 veces en el período.